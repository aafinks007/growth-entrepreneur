...
const ClientsManager = () => {
  ...
  const fetchClients = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('clients').select('*').order('order_index', { ascending: true }).order('id');
  ...
  const handleAddSubmit = async (e) => {
    ...
    let error;
    if (isEditing) {
      const res = await supabase.from('clients').update(payload).eq('id', formData.id);
      error = res.error;
    } else {
  ...
  const handleDragEnd = async (event) => {
    ...
    if (active.id !== over.id) {
      ...
      const updates = newOrder.map((client, index) => ({
        id: client.id,
        order_index: index
      }));
      
      for (const update of updates) {
        await supabase.from('clients').update({ order_index: update.order_index }).eq('id', update.id);
      }
    }
  };
      ...
      {isAdding && (
        <form onSubmit={handleAddSubmit} className="glass-panel" style={{ padding: '2rem', marginBottom: '2rem', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ width: '100%', marginBottom: '1rem' }}>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem' }}>Upload Client Logo</label>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '600px' }}>
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={clients.map(c => c.id)} strategy={verticalListSortingStrategy}>
                <tbody>
                  {clients.map(client => (
                    <SortableRow key={client.id} id={client.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      <td style={{ padding: '1rem' }}>
                        {client.logo_url && <img src={client.logo_url} alt={client.name} style={{ width: '40px', height: '40px', objectFit: 'contain', borderRadius: '4px', background: '#fff' }} />}
                      </td>
                      ...
                      <td style={{ padding: '1rem', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        <button 
                          ...
                          title={client.is_visible ? "Hide Client" : "Show Client"}
                          style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: client.is_visible ? '#34c759' : '#ff3b30', display: 'flex', alignItems: 'center', padding: '0.5rem' }}
                        >
